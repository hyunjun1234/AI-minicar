...
int good = 0;
unsigned long previosusMillis=0.0;
...
void adjustDirection(float recoveryBearing) {
  ...
  if (bearingDifference >= 45 && bearingDifference < 90) {
    right();
    delay(500);  // 방향 전환 시간
    straight();  // 직진 상태로 정렬
    return;
...
void loop() {
  ...
  if (Serial2.available() > 0) {
    ...
    if (gps.location.isUpdated()) {
      ...
      // 1초 간격으로 출력
      if (currentMillis - previosusMillis >= interval) {
  ...
  // GPS 모듈로 현재 위치
  if (firstRoadDirection == 1 && currentLatitude != 0.0 && currentLongitude != 0.0) {
    Serial.println("FRD");
  ...
  delay(100);
  if (good == 1 && currentLatitude != 0.0 && currentLongitude != 0.0) {
    Serial.println("NRD");
